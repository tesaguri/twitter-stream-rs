...
pub struct Tweet<'a> {
    ...
    /// Represents the geographic location of this Tweet as reported
    /// by the user or client application.
    ...
    /// *Perspectival* Indicates whether this Tweet has been liked
    /// by the authenticating user.
    pub favorited: Option<bool>,

    /// Indicates the maximum value of the `filter_level` parameter which may
    /// be used and still stream this Tweet. So a value of `Medium` will be
    /// streamed on `None`, `Low`, and `Medium` streams.
    #[serde(borrow)]
    ...
    /// If the represented Tweet is a reply, this field will contain
    ...
    /// If the represented Tweet is a reply, this field will contain
    /// the integer representation of the original Tweet’s author ID. This will
    ...
    /// to the machine-detected language of the Tweet text,
    ...
    /// When present, indicates that the tweet is associated
    /// (but not necessarily originating from) a [Place][1].
    ///
    /// [1]: struct.Place.html
    #[serde(borrow)]
    ...
    /// A set of key-value pairs indicating the intended contextual delivery
    /// of the containing Tweet. Currently used by Twitter’s Promoted Products.
    pub scopes: Option<HashMap<String, JsonValue>>,

    /// Number of times this Tweet has been retweeted.
    pub retweet_count: u64,

    /// *Perspectival* Indicates whether this Tweet has been retweeted
    /// by the authenticating user.
    pub retweeted: bool,

    /// Users can amplify the broadcast of Tweets authored by other users
    /// by [retweeting][1]. Retweets can be distinguished from typical Tweets
    /// by the existence of a `retweeted_status` attribute. This attribute
    ...
    ///
    /// Note that retweets of retweets do not show representations of
    /// the intermediary retweet, but only the original Tweet. (Users can also
    /// [unretweet][2] a retweet they created by deleting their retweet.)
    ///
    ...
    /// The actual UTF-8 text of the status update. See [twitter-text][1]
    ...
    /// shortened, the original text will be available under
    /// the `retweeted_status` object and the `truncated` parameter will be set
    /// to the value of the original status (in most cases, `false`).
    pub truncated: bool,
    ...
    /// When set to `true`, it indicates that this piece of content has been
    /// withheld due to a [DMCA complaint][1].
    ///
    /// [1]: http://en.wikipedia.org/wiki/Digital_Millennium_Copyright_Act
    #[serde(default)]
    pub withheld_copyright: bool,

    /// When present, indicates a list of uppercase
    /// [two-letter country codes][1] this content is withheld from.
